...
#define MCHECK(expr) { if((expr) != 0) return 0; }
#define FURI_PACKED __attribute__((packed))
#define FURI_LOG_E(x,y,...)
#define FURI_LOG_W(x,y,...)
#define FURI_LOG_D(x,y,...)
...
U2fData* u2f_alloc() {
    return (U2fData*)calloc(1, sizeof(U2fData));
}
...
bool u2f_init(U2fData* U2F) {
    ...
    u2f_data_check(false); // Ensure /u2f folder exists before doing FS ops
...
static bool u2f_ecc_sign(mbedtls_ecp_group* grp, const uint8_t* key, uint8_t* hash, uint8_t* signature) {
    mbedtls_mpi r, s, d;
...
static bool u2f_ecc_compute_public_key(
...
static uint16_t u2f_register(U2fData* U2F, uint8_t* buf) {
    ...
    U2fKeyHandle handle;
    uint8_t priv_key_reg[U2F_EC_KEY_SIZE];
    U2fPubKey pub_key;
    uint8_t hash[U2F_HASH_SIZE];
...

    {
        ...
        MCHECK(mbedtls_md_hmac_finish(&hmac_ctx, priv_key_reg));

        MCHECK(mbedtls_md_hmac_reset(&hmac_ctx));

        // Generate private key handle
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, priv_key_reg, sizeof(priv_key_reg)));
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, req->app_id, sizeof(req->app_id)));
    ...
    // Generate public key
    u2f_ecc_compute_public_key(&U2F->group, priv_key_reg, &pub_key);
